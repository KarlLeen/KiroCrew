...
6. **A conflicted PR's checks are stale, not signal — so a conflict means rebase
   ...
   `reviewDecision`.
   **But do not read "exit 10" as "nothing to do":** the script returns 10 for any
   still-running check *before* it ever evaluates the conflict, so a conflicted PR
   with one check in flight exits **10**, and the conflict is only reported as 20
   once everything has settled. That masking is exactly how the indefinite poll
   happens. So on exit 10, still read the printed
   `mergeable= / mergeState= / reviewDecision=` line: on a conflict or `BEHIND`,
   re-sync and re-push instead of polling, and never report the previous head's
   green.
7. **A fully green PR can still be terminally blocked by a human decision.**
   ...
   the checks rollup; `pr_status.py` reports it as 20 once checks settle, and the
   same masking applies while they are running. The judgment the exit code cannot
...
monitor_start(
  message="Check PR #247. FIRST read
           ...
           terminal PR state). For the AI review lane read the JOB LOG for the
           current head SHA, not the check conclusion. Fix legitimate
