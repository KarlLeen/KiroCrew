...
def _refresh_stale_unit(cfg: PodConfig) -> subprocess.CompletedProcess | None:
    """Re-render the template unit and load it; report why the caller must not proceed.

    Returns ``None`` once systemd is running the current definition, or the failing
    ``daemon-reload`` result when it is not.

    The unlink on failure is the load-bearing half. :func:`unit_is_current` reads the
    file on DISK, but what systemd executes is the definition it has LOADED; a failed
    reload splits the two. Leaving the freshly rendered file in place would make every
    later call read "current" while systemd still runs the old ``ExecStopPost`` — so a
    crash restart would wipe the pod's HOME, silently, for as long as nothing else
    happened to reload. Removing the file restores the invariant that a present,
    current-looking unit is one systemd has actually loaded, so the next call
    re-renders and retries instead of trusting a stale cache.
    """
...
def stop_pod(cfg: PodConfig, name: str) -> subprocess.CompletedProcess:
    ...
    with pod_name_mutex(cfg, name):
        ...
        # this path exists to remove. So refresh a stale unit BEFORE stopping it:
        # daemon-reload re-parses the fragment for an already-running unit, and
        # the stop job has not started yet, so the refreshed (hookless)
        # definition is what runs. Without this, the first `down` after an
        # upgrade still loses the pod's sessions and config.
        #
