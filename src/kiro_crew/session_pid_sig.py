...
def _load_hmac_key() -> bytes | None:
    ...
    try:
        ...
        raw = b""
    key = raw if len(raw) >= _HMAC_KEY_MIN_BYTES else _sel_hmac_key_bytes()
    if key is not None:
        # Signing works, so a LATER failure on this same path deserves a fresh
        # operator-facing report rather than the debug line a retained entry
        # would produce. Without this reset a trust root that breaks, is
        # restored, then breaks again is silent — in the one case (long-lived
        # gateway, never restarted) where the log is the only signal there is.
        with _report_lock:
            _signing_unavailable_reported.discard(str(sel_hmac_key_path()))
    return key


# Resolved trust-root paths already reported as unsignable, so the operator-
# facing message is emitted once per path per process instead of once per
# session claim. Guarded by _report_lock: publication runs on concurrent
# session claims, and an unlocked check-then-add lets two of them both pass the
# membership test and emit duplicate reports.
_signing_unavailable_reported: set[str] = set()
_report_lock = threading.Lock()


def _report_signing_unavailable() -> None:
    """Report an unloadable trust root ONCE per resolved path, per process.
    ...
    consequence and the path to fix. Keyed on the path so a genuine relocation
    is reported again rather than suppressed by the first failure.
    """
    path = str(sel_hmac_key_path())
    with _report_lock:
        first = path not in _signing_unavailable_reported
        _signing_unavailable_reported.add(path)
    if not first:
