...
def _owns_its_replay_store(cfg: PodConfig, name: str, mine: Path) -> bool:
    """True when pod *name* is RECORDED as reading a store that is not *mine*.

    The record is written by the boot that launched the pod, from the value it
    exported (:func:`~kiro_crew.pod.runtime.record_replay_store`). Nothing here is
    inferred: a reader that re-derives the path answers for a pod booted by today's
    code, and a running pod may have been launched by an older one, so it would
    write into the shared store while deriving as isolated. A directory existing
    under the pod home is no better — a stale private store survives a version
    rollback that reuses the pod's name.

    No record means the store cannot be established, which counts as sharing. A pod
    is therefore protected unless its own boot proved otherwise.
    """
    store = pod_runtime.recorded_replay_store(cfg, name)
